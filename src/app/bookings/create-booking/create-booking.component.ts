...
export class CreateBookingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.selectedMode  === 'random') {
      this.startDate = new Date (availableFrom.getTime() + Math.random() * (availableTo.getTime() - 7 * 24 * 60 * 60 * 1000
  ...
  datesValid() {
    const startDate1 = new Date(this.form.value['date-from']);
    const endDate1 = new Date(this.form.value['date-to']);
